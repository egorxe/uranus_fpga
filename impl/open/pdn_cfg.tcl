...
# Hacked a bit to skip stripes generation for unused domains on CORE level
if { [lsearch $::env(PDN_NO_STRIPE_DOMAINS) $::env(VDD_NET)] != -1 } {
    puts "Skipping straps for $::env(VDD_NET) & $::env(GND_NET)!"
    define_pdn_grid -name ringonly_grid -starts_with POWER -voltage_domain CORE
    
    #### ugly addition of empty stripes field to avoid crash
    variable current_grid
    set current_grid [pdngen::check_grid [pdngen::get_grid ringonly_grid]]
    set grid $current_grid
    dict append grid straps {}
    set current_grid [pdngen::verify_grid $grid]
    ####
    
    add_pdn_connect -grid ringonly_grid -layers [subst {$::env(FP_PDN_LOWER_LAYER) $::env(FP_PDN_UPPER_LAYER)}]
    add_pdn_ring -grid ringonly_grid -layer [subst {$::env(FP_PDN_LOWER_LAYER) $::env(FP_PDN_UPPER_LAYER)}] \
